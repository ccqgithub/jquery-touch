        ...
        pinch: function(ev) {
            ...
            if (config.pinch) {
                ...
                if (Math.abs(1 - scale) > config.minScaleRate) {
                    var scaleEv = simpleClone(eventObj);
                    //手势放大, 触发pinchout事件
                ...
                if (Math.abs(rotation) > config.minRotationAngle) {
                    var rotationEv = simpleClone(eventObj),
                        eventType;
    ...
    Touch.init = function(element, options) {
        ...
        for (i = 0; i < len; i++) {
            if (element.addEventListener) {
                element.addEventListener(arr[i], handlerOriginEvent, false);
            } else {
                element.attachEvent(arr[i], handlerOriginEvent);
            }
        }

        // add jquery event props
